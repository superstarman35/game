...
export function evaluateEventEligibility(state,event){
  ...
  for(const requirement of event.npcInterestRequirements??[]){const npc=(state.npcs??[]).find(item=>item.id===requirement.npcId),compare=OPERATORS[requirement.operator],actual=npc?.[requirement.stat];if(!npc||!compare||!compare(actual,requirement.value))blocks.push(`NPC_INTEREST_REQUIREMENT:${requirement.npcId}.${requirement.stat}`);else reasons.push(`${npc.name} ${requirement.stat} ${actual}`);}
